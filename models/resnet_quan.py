...
class Activate(nn.Module):
    def __init__(self, wbit_list, quantize=True):
        super(Activate, self).__init__()
        self.wbit_list = wbit_list
        self.abit = self.wbit_list[-1]
        self.acti = nn.ReLU(inplace=True)
        self.quantize = quantize
        if self.quantize:
            self.quan = activation_quantize_fn(self.wbit_list)
...
def quant_refactor(model, custom_config):
    for layer_name, layer_config in custom_config.items():
        layer = getattr(model, 'layers')[int(layer_name[1:])]
        for submodule_name, submodule_config in layer_config.items():
            submodule = getattr(layer, submodule_name)
            for attr_name, attr_value in submodule_config.items():
                submodule.apply(lambda m: setattr(m, attr_name, attr_value))


# For CIFAR10
def resnet20q(wbit_list, abit_list, num_classes):
    # print('rank:', rank, 'groups:', groups)
    
    model = PreActResNet(block=PreActBasicBlockQ, 
                        num_units=[3, 3, 3], 
                        wbit_list=wbit_list, 
                        abit_list=abit_list,
                        num_classes=num_classes, 
                        expand=1)
    # if args.layer_cfg is not None:
    #   quant_refactor(model, args.layer_cfg)

    return model

# For ImageNet
def resnet50q(bit_list, num_classes=1000, expand=4):
